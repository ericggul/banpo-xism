...
function InstancedBlocks({ geometry, material, transforms, castShadow = true, receiveShadow = true }) {
  const ref = useRef();
...
export default function AptComplex116A() {
  const controls = useControls('Apartment', {
    ...
    towerColor: { value: DEFAULT_COLORS.tower, label: 'Tower Color' },
    plinthColor: { value: DEFAULT_COLORS.plinth, label: 'Podium Color' },
  });

  const palette = useMemo(
    () => ({
      tower: controls.towerColor,
  ...
  const zoneMaterials = useMemo(
    () => ({
      tower: new THREE.MeshStandardMaterial({ roughness: 0.68, metalness: 0.04 }),
    }),
    [],
  );
  ...
  useEffect(() => {
    zoneMaterials.tower.color.set(palette.tower);
