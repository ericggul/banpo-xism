...
// Apartment footprint traced from provided floor plan (millimetres, centered on structural core)
const FLOOR_PLAN_MM = [
  [-6862.5, -5830],
  [6862.5, -5830],
  [6862.5, 2645],
...
const FOOTPRINT_BOUNDS_MM = FLOOR_PLAN_MM.reduce(
  (acc, [x, z]) => {
...
const FOOTPRINT_WIDTH = (FOOTPRINT_BOUNDS_MM.maxX - FOOTPRINT_BOUNDS_MM.minX) * MM_TO_M;
const FOOTPRINT_DEPTH = (FOOTPRINT_BOUNDS_MM.maxZ - FOOTPRINT_BOUNDS_MM.minZ) * MM_TO_M;

function createUnitGeometry(height, targetWidth, targetDepth) {
  ...
  const scaleX = targetWidth / FOOTPRINT_WIDTH;
  const scaleZ = targetDepth / FOOTPRINT_DEPTH;
...
function MinimalTower({ config, palette, position }) {
  ...
  const unitGeometry = useMemo(
    () => createUnitGeometry(moduleHeight, moduleWidth, moduleDepth),
    [moduleHeight, moduleWidth, moduleDepth],
  );
  ...
  const unitTransforms = useMemo(() => {
    ...
    for (let floor = 0; floor < floors; floor += 1) {
      ...
      for (let column = 0; column < unitsPerRow; column += 1) {
        ...
        transforms.push({ position: [x, y, zFront], rotation: [0, 0, 0] });
        transforms.push({ position: [x, y, zBack], rotation: [0, Math.PI, 0] });
      }
    }
    return transforms;
    ...
    <group position={position}>
      {unitTransforms.length > 0 && (
        <instancedMesh ref={unitRef} args={[null, null, unitTransforms.length]} castShadow receiveShadow>
          <primitive attach="geometry" object={unitGeometry} />
...
function ApartmentComplex() {
  const {
    ...
    moduleWidth: {
      value: FOOTPRINT_WIDTH,
      min: FOOTPRINT_WIDTH * 0.7,
      max: FOOTPRINT_WIDTH * 1.3,
      step: 0.05,
      label: 'Unit Width (m)',
    },
    moduleHeight: {
    ...
    moduleDepth: {
      value: FOOTPRINT_DEPTH,
      min: FOOTPRINT_DEPTH * 0.7,
      max: FOOTPRINT_DEPTH * 1.3,
      step: 0.05,
      label: 'Unit Depth (m)',
    },
    unitsPerRow: {
