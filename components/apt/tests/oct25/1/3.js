...
function MinimalistApartmentSculpture() {
  ...
  const towerDepth = moduleDepth * 2.6;
  ...
  const cellTransforms = useMemo(() => {
    const transforms = [];
    const halfModuleDepth = (moduleDepth * 0.54) / 2;
    const facadeInset = moduleDepth * 0.16;
    const depthOffsets = [-towerDepth / 2 + halfModuleDepth + facadeInset, towerDepth / 2 - halfModuleDepth - facadeInset];
    depthOffsets.forEach((z) => {
    ...
    return transforms;
  }, [columns, floors, moduleHeight, moduleDepth, spacingX, spacingY, towerBaseY, towerDepth, towerWidth]);
  ...
  const slitTransforms = useMemo(() => {
    ...
    return transforms;
  }, [floors, moduleDepth, spacingY, towerBaseY, towerWidth, towerDepth]);
